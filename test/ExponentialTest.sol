...
contract ExponentialTest is Exponential, AssertHelpers {
    ...
    function testGetExp_Simple() public {
        (Error err, Exp memory val) = getExp(50, 10000); // 50 basis points
    ...
    function testGetExp_WithDenomOfOne() public {
        (Error err, Exp memory val) = getExp(5, 1); // The number 5.0
    ...
    function testGetZero_Zero() public {
        (Error err, Exp memory val) = getExp(0, 10000); // 0 basis points
    ...
    function testGetExp_FailToGetDivByZero() public {
        (Error err, Exp memory val) = getExp(1, 0);

        assertError(Error.DIVISION_BY_ZERO, err, "divide by zero");
        assertZero(val.mantissa, "default value");
    }

    function testGetExp_Overflow() public {
        (Error err, Exp memory val) = getExp(uint(-1), uint(-1)); // 1, but overflows

        assertError(Error.INTEGER_OVERFLOW, err, "overflows max int");
        assertZero(val.mantissa, "default value");
    }

    function testAddExp_Successful() public {
        (Error err0, Exp memory val1) = getExp(50, 10000); // 50 basis points
        assertNoError(err0);

        (Error err1, Exp memory val2) = getExp(60, 10000); // 60 basis points
        assertNoError(err1);

        (Error err2, Exp memory sum) = addExp(val1, val2);
        assertNoError(err2);
    ...
    function testAddExp_Overflow() public {
        ...
        (Error err, Exp memory sum) = addExp(val1, val2);

        assertError(Error.INTEGER_OVERFLOW, err, "overflowed with addition");
    ...
    function testSubExp_Successful() public {
        ...
        (Error err2, Exp memory difference) = subExp(val2, val1);
        assertNoError(err2);

        Assert.equal(1000000000000000, difference.mantissa, "should be 10 basis points");

        // -1 - (-1) should actually work out to 0
        (Error err3, Exp memory difference2) = subExp(Exp({mantissa: uint(-1)}), Exp({mantissa: uint(-1)}));
        assertNoError(err3);
    ...
    function testSubExp_Underflow() public {
        (Error err0, Exp memory val1) = getExp(5, 1);
        assertNoError(err0);

        (Error err1, Exp memory val2) = getExp(7, 1);
        assertNoError(err1);

        (Error err2, Exp memory difference) = subExp(val1, val2); // 5 - 7 = underflow

        assertError(Error.INTEGER_UNDERFLOW, err2, "underflowed with subtraction");
        assertZero(difference.mantissa, "default value");
    }

    function testMulExp_Successful() public {

        (Error err0, Exp memory val1) = getExp(50, 100); // 1/2
        assertNoError(err0);

        (Error err1, Exp memory val2) = getExp(202, 100); // 2.02
        assertNoError(err1);

        (Error err2, Exp memory product) = mulExp(val1, val2);
    ...
    function testMulExp_RoundAtFarRight() public {
        ...
        (Error err0, Exp memory val1) = getExp(2, 3); // 1/2
        assertNoError(err0);

        (Error err1, Exp memory val2) = getExp(1, 10**18); // 1x10e-18
        assertNoError(err1);
    ...
    function testMulExp_ZeroLeft() public {
        ...
        (Error err0, Exp memory left) = getExp(0, 100); // 0
        assertNoError(err0);

        (Error err1, Exp memory right) = getExp(202, 100); // 2.02
        assertNoError(err1);

        (Error err2, Exp memory product) = mulExp(left, right);
    ...
    function testMulExp_ZeroRight() public {
        ...
        (Error err0, Exp memory right) = getExp(0, 100); // 0
        assertNoError(err0);

        (Error err1, Exp memory left) = getExp(202, 100); // 2.02
        assertNoError(err1);
    ...
    function testMulExp_OneLeft() public {
        ...
        (Error err0, Exp memory left) = getExp(1, 1); // 1
        assertNoError(err0);
    ...
    function testMulExp_OneRight() public {
        ...
        (Error err0, Exp memory right) = getExp(1, 1); // 1
        assertNoError(err0);
    ...
    function testMulExp_Overflow() public {
        ...
        (Error err, Exp memory product) = mulExp(val1, val2);

        assertError(Error.INTEGER_OVERFLOW, err, "overflowed with multiplication");
    ...
    function testMulExp_OverflowOnAddHalfScale() public {
        ...
        assertError(Error.INTEGER_OVERFLOW, err, "should have returned Error.INTEGER_OVERFLOW");
    ...
    function testMulScalar_Big() public {
        ...
        (Error err0, Exp memory scaled1) = mulScalar(val, 5);
        assertNoError(err0);
        Assert.equal(uint(5 * 10**76), scaled1.mantissa, "scalar multiplication failed- scaled1.mantissa not 50e76");

        (Error err1, Exp memory scaled2) = mulScalar(val, 100);
        assertError(Error.INTEGER_OVERFLOW, err1, "should have overflowed with scalar multiplication");
    ...
    function testMulScalar_Small() public {
        ...
        (Error err0, Exp memory scaled) = mulScalar(val, 5);
        assertNoError(err0);
    ...
    function testDivScalar_Big() public {
        ...
        (Error err0, Exp memory scaled1) = divScalar(val, 5);
        assertNoError(err0);
        Assert.equal(uint(2 * 10**75), scaled1.mantissa, "scalar division failed- scaled1.mantissa not 2e76");

        (Error err1, Exp memory scaled2) = divScalar(val, 0);
        assertError(Error.DIVISION_BY_ZERO, err1, "should have caused division by zero");
    ...
    function testDivScalar_Small() public {
        ...
        (Error err0, Exp memory scaled) = divScalar(val, 5);
        assertNoError(err0);
    ...
    function testDivExp() public {
        ...
        (err, result) = divExp(left, right);
        Assert.equal(uint(err), uint(Error.DIVISION_BY_ZERO), "Exp division by 0 should have returned Error.DIVISION_BY_ZERO");
    ...
    function testDivScalarByExp() public {
        ...
        Assert.equal(uint(err), uint(Error.INTEGER_OVERFLOW), "scalar >= 2**237 should cause overflow when converted to Exp");

        ////////////////////
        // division by zero
        (err, result) = divScalarByExp(10, getExpFromRational(0, 1));

        Assert.equal(uint(err), uint(Error.DIVISION_BY_ZERO), "division by zero should return error DIVISION_BY_ZERO");
    }
